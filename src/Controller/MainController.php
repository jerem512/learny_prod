...
class MainController extends AbstractController
{
     ...
     * @Route("/", name="home")
     */
    public function index(ReportBugRepository $reportBugRepository, ClosingRateRepository $closingRateRepository, CloseRepository $closeRepository, CheckOfferService $checkOfferService, FiveEventsService $fiveEventsService, ObjectivesRepository $objectivesRepository, EntityManagerInterface $entityManager): Response
    {
        ...
        foreach($sales as $sale){
            ...
            $ca[] = floatval($offer->getPriceHt());
        }

        $percent_week = array_sum($salary) / $objectives->getGoalWeek() * 100;
        $percent_month = array_sum($salary) / $objectives->getGoalMonth() * 100;
        $percent_year = array_sum($salary) / $objectives->getGoalYear() * 100;
